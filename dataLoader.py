...
def load_init_data(dataname, datadir):
    if dataname == 'mnist':
        ...
        transform_train = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.2860,), (0.3530,))  # Normalization parameters for FashionMNIST
        ])
...
def partition_data(dataname, datadir, partition, n_nets, alpha):
    ...
    if partition == "homo":
        ...
        while (min_size < 10) or (dataname == 'mnist' and min_size < 100) or (dataname == 'fashion-mnist' and min_size < 100):
...
def poisoning_dataset(dataname, data_for_poison, trigger_label, poison_idx):
    remain = []
    if len(poison_idx):
        if dataname == 'mnist' or dataname == 'fashion-mnist' or dataname == 'emnist':
...
def create_train_data_loader(dataname, train_data, trigger_label, posioned_portion, batch_size, dataidxs, malicious=True):
    if malicious == True:
        ...
        if posioned_portion == 0:
            ...
            poison_idx = perm
            clean_idx = []
        elif len(perm) == 1 and 0 < posioned_portion < 1:
            poison_idx = [perm[0]]
            clean_idx = []
        else:
            perm = perm.tolist()
