...
ALL_METHODS = [
     'PlainCSI','FL_TDoS','FL_CPE','fix-1','fix-2','fix-frequency','fix-pool'
]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_nets', type=int, default=100, help='number of totally available users')
    parser.add_argument('--part_nets_per_round', type=int, default=10, help='number of participating clients per FL round')
    ...
    parser.add_argument('--client_select', type=str, default='fix-1', help='the strategy for PS to select client',choices=ALL_METHODS)
    ...
    parser.add_argument('--poisoned_portion', type=float, default=0.4, help='posioning portion (float, range from 0 to 1, default: 0.1)')
    ...
    parser.add_argument('--backdoor_type', type=str, default="none", help='backdoor type used: none|trigger|semantic|edge-case|replacement')
    ...
    parser.add_argument('--power', type=int, default=10000,
    ...
    parser.add_argument('--attacker_index',type=int, default=16,help = 'attacker index in the H,attacker_index <= K')
    ...
    parser.add_argument('--defense_method', type=str, default="none",help='defense method used: none|fedavg|krum|multi-krum|xmam|bulyan|ndc|rsa|rfa|trimmedmean|median|fltrust|')
    # "trimmedmean"  "median" "clustering" "bulyankrum"  "Fltrust" 
    #############################################################################
    ...
    ###################################################################################### select networks
    if args.model == "lenet":
        if args.load_premodel==True and args.dataname == 'mnist':
        ...
        elif args.load_premodel==True and args.dataname == 'fashion-mnist':
            net_avg = LeNet().to(device)
            with open("savedModel/fmnist_lenet.pt", "rb") as ckpt_file:
                ckpt_state_dict = torch.load(ckpt_file, map_location=device)
            net_avg.load_state_dict(ckpt_state_dict)
            logger.info("Loading pre-model successfully ...")
        else:
    ...
    arguments = {
        ...
        "conspirator_idx":args.conspirator_idx,

    }


    # The main function of FL
